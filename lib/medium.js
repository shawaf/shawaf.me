...
async function fetchMediumArticle(slug) {
  ...
  const candidates = [
    `https://${mediumProfileDomain}/${slug}?format=json`,
    `https://medium.com/@${mediumUsername}/${slug}?format=json`,
  ];
    ...
    for (const url of candidates) {
      try {
        ...
        return {
          ...
          link: `https://${mediumProfileDomain}/${slug}`,
...
export async function getMediumPostBySlug(slug) {
  ...
  return fetchMediumArticle(slug);
}
