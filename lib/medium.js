...
async function fetchMediumFeed(url, { timeoutMs = 5000 } = {}) {
  ...
  try {
    ...
    const attempts = await Promise.allSettled(
      candidates.map(async (url) => {
        try {
          const feed = await fetchMediumFeed(url, { timeoutMs: 5000 });
    ...
    const firstSuccessful = attempts.find(
      (result) => result.status === "fulfilled" && result.value?.length
    );

    const feedItems = firstSuccessful?.value;

    if (!feedItems?.length) {
      return [];
    }

    return feedItems.slice(0, limit).map((item, index) => {
